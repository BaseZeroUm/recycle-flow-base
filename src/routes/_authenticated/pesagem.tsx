...
import { useMateriais, useMovimentacoes, useParceiros, type Movimentacao } from "@/lib/dados";
...
export const Route = createFileRoute("/_authenticated/pesagem")({
  head: () => ({
    meta: [
      { title: "Pesagem e tickets — Base 01" },
      {
        name: "description",
        content: "Emita tickets de balança com peso bruto, tara e valor total automático.",
      },
      { property: "og:title", content: "Pesagem e tickets — Base 01" },
      {
...
function Pesagem() {
  ...
  const salvar = useMutation({
    ...
    onSuccess: (mov) => {
      toast.success(`Ticket nº ${mov.numero_ticket} emitido`);
    ...
    onError: (e: Error) => toast.error("Erro ao emitir ticket", { description: e.message }),
  });
  ...
  const pesoMedio = movs.length
    ? movs.reduce((s, m) => s + Number(m.quantidade), 0) / movs.length
    : 0;

  return (
    <div>
      <PageHeader
        titulo="Pesagem e tickets"
        descricao="Toda entrada e saída de material nasce de um ticket de balança."
        acoes={
          <Dialog open={aberto} onOpenChange={setAberto}>
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Novo ticket de pesagem</DialogTitle>
              </DialogHeader>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard destaque label="Tickets emitidos" valor={num(movs.length, 0)} />
        <StatCard
          label="Entradas x saídas"
          valor={`${entradas.length} / ${saidas.length}`}
          detalhe="Compras e vendas"
        />
        <StatCard label="Peso médio por ticket" valor={`${num(pesoMedio)} kg`} />
      </div>
              ...
              <TableRow key={m.id}>
                ...
                <TableCell className="text-right">
                  <Button size="sm" variant="ghost" onClick={() => setTicketAberto(m)}>
                    <TicketIcon className="h-4 w-4" /> Ver
                  </Button>
                </TableCell>
              </TableRow>
            ))}
